...
import { generateImages, improvePrompt } from "@/lib/helpers";

const utapi = new UTApi();
export const maxDuration = 300;

export async function POST(request: NextRequest) {
  const { businessName, slogan, industry, colorPalette, logoType, userId } =
    await request.json();
  ...
  try {
    ...
    const refinedPrompt = await improvePrompt(
      businessName,
      slogan,
      industry,
      colorPalette,
