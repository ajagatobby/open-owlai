...
const steps = ["Upload Image", "Generate Anime"];
const MAX_FILE_SIZE = 3 * 1024 * 1024; // 3MB in bytes

type PhotoToAnimeInputReadonly =
  (typeof REPLICATE_MODELS)["PHOTO_TO_ANIME"]["input"];

type PhotoToAnimeInput = {
  -readonly [K in keyof PhotoToAnimeInputReadonly]: PhotoToAnimeInputReadonly[K] extends
    | string
    | number
    | boolean
    ? PhotoToAnimeInputReadonly[K]
    : string | number | boolean;
};

const PhotoToAnimePage: React.FC = () => {
  const { runModel, result, error, isLoading } =
    useReplicateModels("PHOTO_TO_ANIME");
  const [currentStep, setCurrentStep] = useState(0);
  const [formData, setFormData] = useState<PhotoToAnimeInput>({
    image: "",
    width: 1024,
    height: 1024,
    prompt: "anime",
  ...
  const handleNext = async () => {
    if (currentStep < steps.length - 1 && isStepComplete) {
      ...
      await runModel(formData as PhotoToAnimeInputReadonly);
    }
  };
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  <p className="text-sm text-blue-700">
                    Please upload a photo to convert to anime style. The model
          ...
          <div className="space-y-4">
            {resultImage ? (
              ...
              <div className="text-center">
                <p>Click "Generate" to create your anime-style image!</p>
              </div>
            )}
          </div>
        );
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col text-gray-800">
      <Head>
        <title>Photo to Anime Conversion</title>
        <meta
          name="description"
          content="Convert photos to anime style with AI"
        />
      </Head>
...
export default PhotoToAnimePage;
