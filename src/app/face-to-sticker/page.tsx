...
const steps = ["Upload Image", "Generate Sticker"];
const MAX_FILE_SIZE = 3 * 1024 * 1024; // 3MB in bytes

type FaceToStickerInputReadonly =
  (typeof REPLICATE_MODELS)["FACE_TO_STICKER"]["input"];

type FaceToStickerInput = {
  [K in keyof FaceToStickerInputReadonly]: FaceToStickerInputReadonly[K] extends
    | string
    | number
    | boolean
    ? FaceToStickerInputReadonly[K]
    : FaceToStickerInputReadonly[K] extends ReadonlyArray<infer U>
    ? U[]
    : FaceToStickerInputReadonly[K] extends object
    ? {
        -readonly [P in keyof FaceToStickerInputReadonly[K]]: FaceToStickerInputReadonly[K][P];
      }
    : FaceToStickerInputReadonly[K];
};

const FaceToStickerPage: React.FC = () => {
  const { runModel, result, error, isLoading } =
    useReplicateModels("FACE_TO_STICKER");
  const [currentStep, setCurrentStep] = useState(0);
  const [formData, setFormData] = useState<FaceToStickerInput | any>({
    image: "",
    steps: 20,
    width: 1024,
    height: 1024,
  ...
  const handleNext = async () => {
    if (currentStep < steps.length - 1 && isStepComplete) {
      ...
      setResultImages([]);
      await runModel(formData as FaceToStickerInputReadonly);
    }
  };
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  <p className="text-sm text-blue-700">
                    Please upload an image containing a face. The model works
                    best with clear, front-facing portraits.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4">
            {resultImages.length > 0 ? (
              ...
              <div className="text-center">
                <p>Click "Generate" to create your stickers!</p>
              </div>
            )}
          </div>
        );
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col text-gray-800">
      <Head>
        <title>Face to Sticker Conversion</title>
        <meta
          name="description"
          content="Professional face to sticker conversion tool"
        />
      </Head>
...
export default FaceToStickerPage;
