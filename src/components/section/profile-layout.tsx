...
import { Zap, Star, Crown, ChevronRight, Check, X } from "lucide-react";
...
import { useManageSubscription } from "@/lib/hooks/useManageSubscription";
import { useUpgradeSubscription } from "@/lib/hooks/useUpgradeSubscription";
...
const subscriptionPlans = [
  {
    ...
    productId: process.env.NEXT_PUBLIC_BASIC_PRODUCT_ID as string,
    monthlyPriceId: process.env.NEXT_PUBLIC_BASIC_MONTHLY_PRICE_ID as string,
    yearlyPriceId: process.env.NEXT_PUBLIC_BASIC_YEARLY_PRICE_ID as string,
  },
  {
    name: "Professional",
    futurePrice: 45,
    ...
    productId: process.env.NEXT_PUBLIC_STANDARD_PRODUCT_ID as string,
    monthlyPriceId: process.env.NEXT_PUBLIC_STANDARD_MONTHLY_PRICE_ID as string,
    yearlyPriceId: process.env.NEXT_PUBLIC_STANDARD_YEARLY_PRICE_ID as string,
    recommended: true,
  },
  {
    name: "Premium",
    ...
    productId: process.env.NEXT_PUBLIC_PREMIUM_PRODUCT_ID as string,
    monthlyPriceId: process.env.NEXT_PUBLIC_PREMIUM_MONTHLY_PRICE_ID as string,
    yearlyPriceId: process.env.NEXT_PUBLIC_PREMIUM_YEARLY_PRICE_ID as string,
  },
];

const ProfileLayout: React.FC<{ subscription?: Subscription }> = ({
  subscription,
}) => {
  ...
  const { user } = useUser();
  const { mutate: upgradeSubscription, isUpgrading } = useUpgradeSubscription();

  const { openCheckout } = usePaddle({
    environment: process.env.NEXT_PUBLIC_PADDLE_ENVIRONMENT as
      | "sandbox"
      | "production",
    token: process.env.NEXT_PUBLIC_PADDLE_CLIENT_SECRET!,
    eventCallback: (data) => {
  ...
  const { fetchManageSubscription, manageSubscription } =
    useManageSubscription();

  const handleManageSubscription = async (subscriptionId: string) => {
    try {
      await fetchManageSubscription(subscriptionId);
      router.push(manageSubscription?.update_url as string);
    } catch (error) {
  ...
  const handleCancelSubscription = async (subscriptionId: string) => {
    try {
      await fetchManageSubscription(subscriptionId);
      router.push(manageSubscription?.cancel_url as string);
    } catch (error) {
  ...
  const handleSubscribe = async (plan: (typeof subscriptionPlans)[0]) => {
    ...
    const checkoutOptions = {
      items: [
        {
          priceId: annualBilling ? plan.yearlyPriceId : plan.monthlyPriceId,
          quantity: 1,
        },
      ],
      customer: {
    ...
    try {
      openCheckout(checkoutOptions as any);
    } catch (error) {
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            <p className="text-center text-gray-600 mb-8 max-w-2xl mx-auto">
              ...
              plans. Find the perfect fit for your logo design needs.
            </p>
                  ...
                  <div className="p-8">
                    ...
                    <AnimatePresence>
                      ...
                      subscription?.plan.toLowerCase() ===
                        ...
                        >
                          <Button
                            className="w-full py-4 h-12 text-lg font-medium rounded-xl mb-2 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 transition-all duration-300"
                            onClick={() =>
                          ...
                          <Button
                            variant="outline"
                            className="w-full py-4 h-12 text-lg font-medium rounded-xl text-red-500 border-red-500 hover:bg-red-50 transition-all duration-300"
                            onClick={() =>
